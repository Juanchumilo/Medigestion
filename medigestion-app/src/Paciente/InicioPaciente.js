...
export default function InicioPaciente() {
    ...
    if (vistaActiva === 'crear') {
        return <CreadorCitas />; // All the messy logic is hidden inside this file!
    } 
    if (vistaActiva === 'buscar') {
        return <BuscadorCitas />; // Same here
    }
    // Default view
    return <Text style={styles.textoWelcome}>Bienvenido a tu panel de Paciente.</Text>;
    };
    ...
    return (
        ...
        <View style={[GlobalStyles.container, {alignItems:'center'}]}>
            <View style={[GlobalStyles.container, {alignItems:'center', justifyContent:'center'}]}>
                <Image source={require('../../assets/images/medigestion.png')} style={[GlobalStyles.logo, {marginBottom:20}]}/>
                <Text style={styles.textoWelcome}>Hola, {capitalizarNombre(perfil?.nombre)}</Text>
                <Text style={styles.subtitle}>¿Qué deseas hacer el día de hoy?</Text>

                //* The Navigation Buttons *//
                <View style={styles.container2}>
                    <TouchableOpacity style={[GlobalStyles.buttonMain, {marginTop:10, marginBottom:24}]} onPress={() => setVistaActiva('crear')}>
                    <Text style={[GlobalStyles.textbutton]}>Crear Cita</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={[GlobalStyles.buttonMain]} onPress={() => setVistaActiva('buscar')}>
